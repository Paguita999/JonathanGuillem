...
*/
    public ResultadoCarrera(Element result){        
        Element driverElem = (Element) result.getElementsByTagName("Driver").item(0);
        this.d = new Driver(driverElem);

        Element constructorElem = (Element) result.getElementsByTagName("Constructor").item(0);
        this.constructor = constructorElem.getElementsByTagName("Name").item(0).getTextContent();
        ...
        Element timeElem = (Element) result.getElementsByTagName("Time").item(0);
        if (timeElem != null && timeElem.hasAttribute("millis")) {
            this.timeMillis = Long.parseLong(timeElem.getAttribute("millis"));
        } else {
            this.timeMillis = 0;
        }

        Element statusElem = (Element) result.getElementsByTagName("Status").item(0);
        this.finisher = statusElem != null && "Finished".equals(statusElem.getTextContent());

        Element fastestLapElem = (Element) result.getElementsByTagName("FastestLap").item(0);
        if (fastestLapElem != null && fastestLapElem.hasAttribute("rank")) {
            this.rankFastesLap = Integer.parseInt(fastestLapElem.getAttribute("rank"));
        } else {
            this.rankFastesLap = -1;
        }
    }   
